...
class ProposalController {
  ...
  static async getProposals(req, res) {
    try {
      ...
      if (!userId && !vendorId) {
        return res.status(400).json({ message: "Provide at least userId or vendorId in query" });
      }
  
      const results = await ProposalManager.getProposalsByFilter({ userId, vendorId });
